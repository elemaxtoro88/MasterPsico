...
REGLA CRÍTICA: Genera un feedback empático y estrategias de afrontamiento ÚNICAS para cada entrada. Evita frases genéricas o repetitivas. Inspírate en los detalles específicos del texto del usuario.`;
    ...
    try {
      ...
      console.log("Client initialized, calling Gemini (v2 SDK)...");
      const result = await client.models.generateContent({
        model: "gemini-1.5-flash",
        contents: `Analiza psicológicamente la siguiente entrada del usuario:\n\n"${text}"`,
        config: {
          systemInstruction,
          ...
          responseSchema: {
            ...
            properties: {
              ...
              empatheticFeedback: { type: "STRING" },
      ...
      if (!result.text) {
        console.warn("Gemini result.text was empty. Checking candidates...");
        if (result.candidates && result.candidates[0]?.content?.parts && result.candidates[0].content.parts[0]?.text) {
          responseText = result.candidates[0].content.parts[0].text;
          console.log("Manual extraction from candidates successful.");
        } else {
          throw new Error("No se pudo obtener texto de Gemini.");
        }
      }
      console.log("Final response text ready for parsing.");
...
REGLA CRÍTICA: No des el mismo consejo dos veces. Busca ángulos originales basados en los matices del perfil del usuario. Evita los clichés.`;
    ...
    try {
      ...
      const response = await client.models.generateContent({
        model: "gemini-1.5-flash",
        contents: `Genera un consejo terapéutico basado en este resumen:\n\n${summary}`,
        config: {
          systemInstruction,
          responseMimeType: "application/json",
          responseSchema: {
...
Responde estrictamente en español y en formato JSON.`;
    ...
    try {
      ...
      const response = await client.models.generateContent({
        model: "gemini-1.5-flash",
        contents: `El usuario completó la reflexión "${templateName}". Respuestas:\n\n${reflectionContent}`,
        config: {
          systemInstruction,
          responseMimeType: "application/json",
          responseSchema: {
